...
class Pdu
{
	...
	public function generate()
	{
		...
		for ($part = 1; $part <= ceil($messageLength / $maxMessageLength); $part++) {
			$message = mb_substr(
				$this->message, ($part - 1) * $maxMessageLength,
				$maxMessageLength,
				self::MESSAGE_CHARSET
			);
			$command[] =
				$this->getSca() .
				$this->pduType() .
	...
	protected function tpUdh($ied1, $part)
	{
		...
		$udh = [
			...
			$this->isUnicodeMessage() ? '03' : '04', // IEDL. Length of the header. IEI = 00 -> 03, IEI = 08 -> 04.
			sprintf($this->isUnicodeMessage() ? '%02X' : '%04X', $ied1), // Reference number.
			sprintf('%02X', ceil(mb_strlen(
					$this->message,
					self::MESSAGE_CHARSET
				) / $maxMessageLength)), // Count of parts.
